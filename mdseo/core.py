...
__all__ = ['get_meta', 'meta_list', 'find_dupe', 'chk_dupe_title', 'chk_desc', 'chk_slug', 'chk_slug_len', 'chk_img',
           'chk_author', 'chk_len', 'chk_desc_len']
...
# Cell
def _has_no_slug(d):
    if 'slug' in d['mdseo-ignore']: return False
    else: return 'slug' not in d

@call_parse
def chk_slug(srcdir:str='.', # directory of files to check
            ):
    "Check if docs do not have the field `slug` in their front matter. Ignore with front matter `mdseo-ignore: [slug]`"
    return _checker(_has_no_slug, "do not have the field `slug` in their front matter", srcdir)

# Cell
def _slug_len(d, n):
    if 'slug' in d['mdseo-ignore']: return False
    else: return len(d.get('slug', '')) > n

@call_parse
def chk_slug_len(srcdir:str='.', # directory of files to check
                 n=45 # max number of characters for slug
            ):
    "Check if docs have a `slug` field in their front matter that is less than `n` characters. Ignore with front matter `mdseo-ignore: [slug]`"
    return _checker(partial(_slug_len, n=n), f"have `slug` in their front matter that is greater than {n} characters", srcdir)
...
def chk_img(srcdir:str='.', # directory of files to check
    ...
    return _checker(_has_no_img, "do not have the field `image` in their front matter", srcdir)

# Cell
def _has_no_author(d):
    if 'authors' in d['mdseo-ignore'] or 'author' in d['mdseo-ignore']: return False
    return 'authors' not in d

@call_parse
def chk_author(srcdir:str='.', # directory of files to check
            ):
    "Check if docs do not have the field `authors` in their front matter. Ignore with front matter `mdseo-ignore: [authors]`"
    return _checker(_has_no_img, "do not have the field `authors` in their front matter", srcdir)
